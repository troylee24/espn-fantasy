var names_tables, headers, columns, records_tables, grades_tables;

function initVars(names_tables, headers, columns, records_tables, grades_tables) {
    names_tables = names_tables;
    headers = headers;
    columns = columns;
    records_tables = records_tables;
    grades_tables = grades_tables;
}
...
$(document).ready(function() {
    for (i=0; i<records_tables.length; i++) {
        let records = records_tables[i];
        let table_name = "#table_" + names_tables[i];
            ...
            "columnDefs": [
                ...
                },
                {
                    ...
                    "createdCell": function (td, cellData, rowData, row, col) {
                        let key = headers[col]
                        let color = grades_tables[i][row][key]
        ...
        let search_bar_name = table_name + "_filter input";
        $(search_bar_name)
            .off()
            .on('keyup', function() {
                console.log(table)
